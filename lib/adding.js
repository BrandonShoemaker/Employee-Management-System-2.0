...
function postEmployee(server){
    ...
    .then(results => {
        ...
        if(results.manager != "NONE")
            managerName = results.manager.split(" "); 


        db.query(`SELECT id FROM roles WHERE name='${results.role}'`, (err, row) => {
            if(err){
                console.log(err);
                return;
            }
        ...
        setTimeout(() => {
            if(results.manager != "NONE"){    
                db.query(`SELECT id FROM employees WHERE first_name='${managerName[0]}' AND last_name='${managerName[1]}'`, (err, row) => {
                    if(err){
                        console.log(err);
                        return;
                    }
...
function getManagersNames(){
    ...
    db.query('SELECT CONCAT(managers.first_name, " ", managers.last_name) AS managers FROM employees LEFT JOIN employees managers ON managers.id = employees.manager_id', (err, rows) => {
        if(err){
            return;
        }
        row = rows;
